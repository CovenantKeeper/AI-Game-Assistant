...
namespace TheCovenantKeepers.AI_Game_Assistant
{
    ...
    {
        public static readonly List<AbilityData> Abilities = new List<AbilityData>();

        public static void LoadAbilityMasterlist(string assetCsvPath)
        {
            Abilities.Clear();

            if (string.IsNullOrEmpty(assetCsvPath) || !File.Exists(assetCsvPath))
            {
                Debug.LogWarning($"AbilityDatabase: CSV not found at '{assetCsvPath}'.");
                return;
            }

            var lines = File.ReadAllLines(assetCsvPath);
            if (lines.Length == 0) { Debug.LogWarning("AbilityDatabase: CSV is empty."); return; }
            if (lines.Length > 1 && lines[1].TrimStart().StartsWith("{"))
            {
                Debug.LogError("AbilityDatabase: File seems to contain a JSON error response, not CSV. Regenerate after fixing API settings.");
                return;
            }
            ...
            for (int i = 1; i < lines.Length; i++)
            {
                ...
                var data = CreateFromRow<AbilityData>(norm, values);
                if (data != null) Abilities.Add(data);
            }

            Debug.Log($"AbilityDatabase now contains {Abilities.Count} entries.");
        }

        // generic helpers (same pattern)
        private static T CreateFromRow<T>(string[] normHeader, string[] values) where T : ScriptableObject
        {
            ...
            {
                Debug.LogWarning($"AbilityDatabase row parse failed: {ex.Message}");
                return null;
            }
        }
