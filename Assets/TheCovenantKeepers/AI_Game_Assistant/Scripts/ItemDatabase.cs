...
namespace TheCovenantKeepers.AI_Game_Assistant
{
    ...
    {
        public static readonly List<ItemData> Items = new List<ItemData>();

        public static void LoadItemMasterlist(string assetCsvPath)
        {
            Items.Clear();

            if (string.IsNullOrEmpty(assetCsvPath) || !File.Exists(assetCsvPath))
            {
                Debug.LogWarning($"ItemDatabase: CSV not found at '{assetCsvPath}'.");
                return;
            }

            var lines = File.ReadAllLines(assetCsvPath);
            if (lines.Length == 0) { Debug.LogWarning("ItemDatabase: CSV is empty."); return; }

            // Guard against JSON error bodies accidentally saved as CSV
            if (lines.Length > 1 && lines[1].TrimStart().StartsWith("{") )
            {
                Debug.LogError("ItemDatabase: File seems to contain a JSON error response, not CSV. Regenerate after fixing API settings.");
                return;
            }
            ...
            for (int i = 1; i < lines.Length; i++)
            {
                ...
                var data = CreateFromRow<ItemData>(norm, values);
                if (data != null) Items.Add(data);
            }

            Debug.Log($"ItemDatabase now contains {Items.Count} entries.");
        }

        // ---------- Generic helpers (same as CharacterDatabase) ----------
        ...
        private static T CreateFromRow<T>(string[] normHeader, string[] values) where T : ScriptableObject
        {
            ...
            {
                Debug.LogWarning($"ItemDatabase row parse failed: {ex.Message}");
                return null;
            }
        }
